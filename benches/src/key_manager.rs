...
use http_private_key_manager::{
    prelude::{KeyGenerator, *},
    private_key_generator::{hkdf::hmac::Hmac, timestamp_policies, CryptoKeyGenerator},
};
...
type KeyGen = KeyGenerator<Hmac<sha3::Sha3_512>, Versioning, ChaCha8Rng, sha3::Sha3_512>;
...
fn bench(c: &mut Benchmarker) {
    ...
    */
    ...
    group.bench_function(BenchmarkId::new("generate_keyless_id", 1), |b| {
        b.iter(|| {
            key_manager
                .generate_keyless_id::<BigId>("prefix", b"test", None, None)
                .unwrap()
        });
    });
    ...
    group.bench_function(BenchmarkId::new("generate_ECDSA_key_id", 1), |b| {
        b.iter(|| {
            key_manager
                .generate_ecdsa_key_and_id::<NistP384, BigId>("prefix", None, None)
                .unwrap()
        });
    });
    ...
    group.bench_function(BenchmarkId::new("validate_ECDSA_key_id", 1), |b| {
        b.iter(|| {
            let result =
                key_manager.validate_ecdsa_key_id::<NistP384, BigId>(&id.0.encoded_id, None);
    ...
    group.bench_function(BenchmarkId::new("sign_with_key_id", 1), |b| {
        b.iter(|| {
            let result =
                key_manager.sign_data_with_key_id::<NistP384, BigId, Sha256>(&[5u8; 1024], &id.0);
            black_box(result.is_ok())
        });
    });
