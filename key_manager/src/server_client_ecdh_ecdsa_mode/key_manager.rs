...
use crate::{
    debug_log,
    error::ProtocolError,
    generated::server_client_ecdh_ecdsa_mode::{decrypt_info::ClientEcdhPubkey, *},
    utils::{b64_len_to_binary_len, padding_trail, StringSanitization},
    Id,
};
#[cfg(feature = "zeroize")]
...
mod tests {
    ...
    #[test]
    fn unassociated_ecdh_keys() {
        let mut key_manager = init_key_manager();
        let server_key = key_manager
            .generate_ecdh_pubkeys_and_ids(1, Some(now!() + years_to_seconds(1)))
            .unwrap()[0]
            .clone();

        let client_id_type = b"test id type";
        let client_id = key_manager
        ...
        let server_shared_secret = key_manager
            .key_generator
            .ecdh_using_key_id::<NistP384, BigId>(
                &server_key.0,
                Some(&client_id.binary_id.as_ref()),
                client_ecdh_key.public_key(),
            );
        assert_eq!(
            initial_shared_secret.raw_secret_bytes(),
    ...
    fn improper_ecdh_key_expiration() {
        ...
        let (_version, timestamp) = key_manager.key_generator.decode_version_and_timestamp_from_id::<BigId>(&key_id);
