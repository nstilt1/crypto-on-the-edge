...
impl EpochCrypto {
    pub fn new(rng: &mut ChaCha8Rng, epoch: u64) -> Self {
        ...
        Self {
            symmetric_key_hkdf: Hkdf::<Sha3_512>::from_prk(&symmetric_kdf_key)
                .expect("Key should be valid"),
            ecdsa_key_hkdf: Hkdf::<Sha3_512>::from_prk(&ecdsa_kdf_key)
                .expect("Key should be valid"),
...
impl ZeroizingCache {
    ...
    pub fn access(&mut self, rng: &mut ChaCha8Rng, epoch: u64) -> &EpochCrypto {
        if let Some(v) = self.cache.get_mut(&epoch) {
            self.access_counts
                .get_mut(&v.access_count)
                .expect("exists")
                .remove(&epoch);
            self.access_counts
                .entry(v.access_count + 1)
                .or_default()
                .insert(epoch);
            v.access_count += 1;
            v
...
impl CryptoOperator {
    pub fn new(key: &[u8]) -> Self {
        ...
        let rng = unsafe {
            switch_stacks(&mut stack, || {
                let hkdf: Hkdf<Sha3_512> =
                    Hkdf::from_prk(key).expect("Main KDF key was not long enough");
                let mut seed = [0u8; 32];
    ...
    /// operation on the separate stack rather than just generating a single
    /// key. In other words, use the key on the separate stack and perform
    /// an individual but larger operation, such as encryption/decryption,
    /// signature, or a key agreement.
    pub fn encrypt_data<A: Aead + KeyInit>(
        &mut self,
        epoch: u64,
        key_id: &[u8],
        data: &mut [u8],
        ...
        unsafe {
            switch_stacks(&mut self.stack, || {
                ...
                epoch_crypto.symmetric_key_hkdf.expand_multi_info(
                    &[b"symmetric key", &epoch.to_le_bytes(), key_id],
                    key.as_mut_slice(),
                );
