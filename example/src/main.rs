...
use http_private_key_manager::{
    private_key_generator::{self, CryptoKeyGenerator},
...
type ExampleKeyManager =
    HttpPrivateKeyManager<ExampleKeyGenerator, NistP384, Sha384, NistP384, ClientId, KeyId, StdRng>;
...
fn initialize_key_manager() -> ExampleKeyManager {
    ...
    let base64_alphabet =
        Alphabet::new("ASDFGHJKLZXCVBNMQWERTYUIOP1234567890asdfghjklqwertyuiopzxcvbnm-_").unwrap();

    let mut secret_key: Secret<[u8; 48]> = Secret::new([42u8; 48]);

    let key_manager = ExampleKeyManager::from_key_generator(
        ExampleKeyGenerator::new(secret_key.as_slice(), b"my arbitrary application ID"),
        base64_alphabet,
        NO_PAD,
    );
}
...
fn begin_handshake() -> Request {
    ...
    // generate ecdh and ecdsa pubkeys and key IDs for the client to use for the
    // first request

    // have the client send a request
    client_side::send_request(
        ecdh_key_id.as_ref(),
        &ecdh_pubkey,
        ecdsa_key_id.as_ref(),
        &ecdsa_pubkey,
...
fn main() {
    ...
    let decrypted_payload =
        key_manager.decrypt_and_hash_request::<HandshakeRequestPayload>(request, true)?;
    ...
    let decrypted_payload =
        key_manager.decrypt_and_hash_request::<OtherPayload>(request_payload, false)?;
}
